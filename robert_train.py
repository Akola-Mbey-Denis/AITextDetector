...
def validation_loop(dataloader,model,loss_fn):
    ...
    with torch.no_grad():
        for batch, dl in loop:
            ...
            if arch =='bert':
                output = model(ids,mask,token_type_ids,labels = label.float())
            else:
                output = model(ids,mask)

            label = label.float()
            loss = loss_fn(output.logits,label)               
            output_ = output.logits.cpu().detach().numpy()
            pred = np.where(output_ >= 0, 1, 0)
...
def training_loop(epochs,dataloader,val_dataloader,model,loss_fn,optimizer):
    ...
    for  epoch in range(1,epochs+1):       
        ...
        for batch, dl in loop:
            ...
            optimizer.step()
            output_ = output.logits.detach().cpu().numpy()
            pred = np.where(output_ >= 0, 1, 0)
        ...
        if epoch%5 == 0:
           ...
           # saved model 
           if epoch>=30:
                torch.save(model.state_dict(), 'start/ai-text-classifier-'+str(val_acc)+'.pth')
...
# scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,milestones=[50,80],gamma=0.33) 
loss_fn = nn.BCEWithLogitsLoss()

model = training_loop(epochs, train_dataloader,val_dataloader, model, loss_fn, optimizer)

torch.save(model.state_dict(), 'start/ai-text-'+arch+str(epochs)+'-classifier-max-length-'+str(max_length)+'.pth')
